...
- **Free** (sirve de verdad): perfil, aparecer en resultados, recibir y ver solicitudes **sin límite**, agenda, reseñas y Tu mes básico, con **10 presupuestos por mes** (`FREE_MONTHLY_QUOTE_LIMIT`). No se limitan servicios, barrios, reseñas, agenda ni solicitudes recibidas.
  ...
  - **UX:** en `/pro/solicitudes`, "Presupuestos este mes · 7 de 10" discreto; desde que quedan 3, "Te quedan 3…" / "Te queda 1…"; en 10/10, "Usaste tus 10 presupuestos de septiembre" + "Ver Resuelve PRO". El presupuesto 10 se envía y avisa sin tapar el éxito. El intento 11 abre un diálogo (bottom sheet en mobile) "Llegaste al límite de Free" con el precio, "Seguir con Free" y "Conocer PRO". Sin modales al entrar ni banners desde 1/10.
...
- **PRO — $19.000 / mes** (`PRO_MONTHLY_PRICE_ARS`): presupuestos sin límite, badge "PRO" en tarjetas y ficha pública (borde Forest), espacios "Destacado" en resultados y Tu mes completo: valor de presupuestos aceptados (no son ingresos), tasa de aceptación (sin enviados "—"), comparación absoluta con el mes anterior, actividad por semana, rendimiento por servicio y por barrio, insights con reglas fijas y **"Tu presencia en Resuelve"**: apariciones en búsquedas, visitas al perfil, embudo (apariciones → visitas → solicitudes → presupuestos → aceptados → realizados) y tasas (visita/aparición, solicitud/visita, aceptación), solo con denominador > 0. Nada de "ROI" ni "personas únicas".
...
- **Sigue fuera:** Mercado Pago/checkout, renovación automática, prueba gratis, plantillas de presupuesto (flag apagado), portfolio ampliado, push y email PRO. Nada de eso se muestra como disponible.
- **Entitlements:** la UI pregunta `ProStore.entitlements()` (`canSendUnlimitedQuotes`, `canBeFeatured`, `canUseAdvancedAnalytics`, `canSeeExposureAnalytics`, `canUseQuoteTemplates`) que manda `/pro/me`, nunca `tier === 'PRO'`. El cupo viene en `/pro/me` → `quoteUsage { period, used, limit, remaining }` (`null` = sin límite). Tu mes usa lo que devuelve el backend (`advanced` y `exposure` en `null` para Free).
- **Página Plan** (`/pro/plan`): "Más oportunidades para aprovechar. Más información para decidir.", precio `$19.000 / mes` y cupo Free desde `GET /plans` (nunca escritos a mano; si `/plans` falla no se muestra precio), tarjetas Free/PRO, "Qué suma PRO" y comparación en tabla de 3 columnas que entra en 390 px. "Contratación online próximamente": PRO se habilita de forma gradual con `npm run plan:set`.
